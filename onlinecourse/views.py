...
def submit(request, course_id):
    ...
    submission.save()
    lesson = submission.choices.first().question.lesson
...
def show_exam_result(request, course_id, lesson_id, submission_id):
    ...
    for question in questions:
        print(submission_id,question.id)
        choices = Choice.objects.filter(question=question).values_list('id',flat=True)
        _selected = Submission.objects.filter(id=submission_id,choices__in = choices).values_list('choices',flat=True)
        score += question.is_get_score(_selected)
        total_points += question.grade
    context['selected'] = selected.values_list('choices',flat = True)
    context['grade'] = 100 * score / total_points
